...
- **🤖 AI 智能打分** — 支持 Claude、GPT-4、Gemini、DeepSeek、豆包（Doubao）、MiniMax、阿里云百炼（DashScope / Qwen）及任何 OpenAI 兼容 API，对每条内容评分 0-10，过滤噪声
...
{
  "ai": {
    "provider": "doubao",       // 支持 "openai", "anthropic", "ali", "gemini", "doubao", "minimax"
...
### 使用阿里云百炼（DashScope / Qwen，OpenAI 兼容模式）

Horizon 对阿里云百炼的 OpenAI 兼容模式做了原生支持：**用法与 OpenAI 完全一致，仅需把 `provider` 设为 `ali`（或继续使用 `openai` 并指定 `base_url`）**。
